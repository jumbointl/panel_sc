...
class IdempiereRESTProvider {
  static final IdempiereRESTProvider _repository = IdempiereRESTProvider._internal();
  late String _baseURL;
  IdempiereSession? _session;
  late String _firstToken;
  int timeOutSeconds = Memory.TIME_OUT_SECOND;

  factory IdempiereRESTProvider() {
    return _repository;
  }

  IdempiereRESTProvider._internal();
  ...
  Map<String, String> _getReqHeaders({String? token}) {
    final IdempiereSession? s = _session;
  ...
  Future<IdempiereSession> oneStepLogin(String endpoint, String userName,
      String password, int clientId, int roleId,
      {int? organizationId, int? warehouseId, String? language}) async {
    _session = IdempiereSession.oneStepLogin(userName, password, clientId, roleId,
    ...
    try {
      ...
      throw APIException('Request timed out', HttpStatus.gatewayTimeout);
    }
   }
  ...
  Future<LoginResponse> login(
    ...
    try {
      var response = await http.post(Uri.parse(_baseURL + endpoint),
          headers: header, body: body).timeout(Duration(seconds: timeOutSeconds));
  ...
  /// return [Session]
  Future<IdempiereSession> initSession(
      String endpoint, String token, int clientId, int roleId,
      {int? organizationId, int? warehouseId, String? language}) async {
    _session = IdempiereSession(token, clientId, roleId,
        organizationId: organizationId,
        warehouseId: warehouseId,
        language: language);
    ...
    try {
      var response = await http.get(
          Uri.parse(_buildUrl(endpoint,
              ...
              context: context,
              showsql: showsql)),
          headers: _getReqHeaders(token: token)).timeout(Duration(seconds: timeOutSeconds));
  ...
  Future put<T extends ModelBase>(String endpoint, T model) async {
    try {
      var response = await http
          .put(Uri.parse("$_baseURL$endpoint/${model.id}"),
              headers: _getReqHeaders(), body: jsonEncode(model.toJson()))
          .timeout(Duration(seconds: timeOutSeconds));
  ...
  Future<IdempiereSession> refresh(
      String endpoint, IdempiereUser user) async {
    _session = IdempiereSession.refresh(user);
    try {
      var response = await http.post(Uri.parse(_baseURL + endpoint),
          headers: _getReqHeaders(), body: jsonEncode(_session!.toJsonRefresh()))
          .timeout(Duration(seconds: timeOutSeconds));
  ...
  Future<bool> logOut(
      String endpoint, IdempiereUser user) async {
    _session = IdempiereSession.idempiereUser(user);
    try {
      var response = await http.post(Uri.parse(_baseURL + endpoint),
          headers: _getReqHeaders(), body: jsonEncode(_session!.toJsonLogOut()))
          .timeout(Duration(seconds: timeOutSeconds));
      if (response.statusCode == HttpStatus.ok) {
        return true;
      } else {
  ...
  Future<IdempiereQueryResult?> getIdempiereLists(String endPoint) async {
    try {
      var response = await http.get(
      ...
      if (response.statusCode == HttpStatus.ok) {
        final fromJson = jsonDecode(utf8.decode(response.bodyBytes));
        if (fromJson == null) return IdempiereQueryResult();

        IdempiereQueryResult queryResult = IdempiereQueryResult.fromJson(fromJson);
        return queryResult;
      } else {
        //final message = jsonDecode(utf8.decode(response.bodyBytes));
        //throw APIException(message["detail"], response.statusCode);
      ...
      // Handle timeout exception specifically for this method if needed
      // For now, let it propagate or return null as per existing logic for other errors
