...
class IdempiereUOMHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereUOM> results = <IdempiereUOM>[].obs;


  IdempiereUOMHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_UOM_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_UOM_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_UOM;
    fields = IdempiereRestCommand.fieldsOfUOM.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereUOM productBrand = IdempiereUOM.fromJson(item);
        results.add(productBrand);
      }
      print('----------------UOM.length: ${results.length}');
      saveIdempiereUOMList(results);
