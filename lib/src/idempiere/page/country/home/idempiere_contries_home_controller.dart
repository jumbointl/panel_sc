...
class IdempiereCountriesHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereCountry> results = <IdempiereCountry>[].obs;


  IdempiereCountriesHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_COUNTRIES_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_COUNTRY_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_COUNTRY;
    fields = IdempiereRestCommand.fieldsOfCountries.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereCountry productBrand = IdempiereCountry.fromJson(item);
        results.add(productBrand);
      }
      print('----------------countries.length: ${results.length}');
      saveIdempiereCountriesList(results);
