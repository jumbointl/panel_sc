...
class IdempiereLocationsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereLocation> results = <IdempiereLocation>[].obs;


  IdempiereLocationsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_LOCATIONS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_LOCATION_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_LOCATION;
    fields = IdempiereRestCommand.fieldsOfLocations.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereLocation productBrand = IdempiereLocation.fromJson(item);
        results.add(productBrand);
      }
      print('----------------locations.length: ${results.length}');
      saveIdempiereLocationsList(results);
