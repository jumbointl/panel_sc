...
class IdempiereCitiesHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereCity> results = <IdempiereCity>[].obs;


  IdempiereCitiesHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_CITIES_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_CITY_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_CITY;
    fields = IdempiereRestCommand.fieldsOfCities.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereCity productBrand = IdempiereCity.fromJson(item);
        results.add(productBrand);
      }
      print('----------------cities.length: ${results.length}');
      saveIdempiereCitiesList(results);
