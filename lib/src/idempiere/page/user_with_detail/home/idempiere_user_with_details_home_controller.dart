...
class IdempiereUserWithDetailsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereUserWithDetail> results = <IdempiereUserWithDetail>[].obs;


  IdempiereUserWithDetailsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_USER_WITH_DETAILS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_USER_WITH_DETAIL_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_USER;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.USER_WITH_DETAILS;
    fields = IdempiereRestCommand.fieldsOfUserWithDetails.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
    ...
    if(endPoint==null){
      ...
      for(var item in fromJson.records!){
        IdempiereUserWithDetail list = IdempiereUserWithDetail.fromJson(item);
        results.add(list);
      }
      print('-----------------------------.length: ${results.length}');
      saveIdempiereUserWithDetailsList(results);
