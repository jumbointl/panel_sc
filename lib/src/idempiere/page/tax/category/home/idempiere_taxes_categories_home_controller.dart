...
class IdempiereTaxesCategoriesHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereTaxCategory> results = <IdempiereTaxCategory>[].obs;


  IdempiereTaxesCategoriesHomeController(){
    endPoint = Memory.IDEMPIERE_ENDPOINT_TAX_CATEGORY;
    routePageDestination = Memory.ROUTE_IDEMPIERE_TAXES_CATEGORIES_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_TAX_CATEGORY_LIST;
    fields = IdempiereRestCommand.fieldsOfTaxesCategories.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      print('----------------TaxesCategories.length: ${results.length}');
      saveIdempiereTaxesCategoriesList(results);
