...
class IdempiereTaxesHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereTax> results = <IdempiereTax>[].obs;


  IdempiereTaxesHomeController(){
    endPoint = Memory.IDEMPIERE_ENDPOINT_TAX;
    routePageDestination = Memory.ROUTE_IDEMPIERE_TAXES_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_TAX_LIST;
    fields = IdempiereRestCommand.fieldsOfTaxes.obs;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.TAXES;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereTax data = IdempiereTax.fromJson(item);
        results.add(data);
      }
      print('----------------Taxes.length: ${results.length}');
      saveIdempiereTaxesList(results);
