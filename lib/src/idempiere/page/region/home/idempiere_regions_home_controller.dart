...
class IdempiereRegionsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereRegion> results = <IdempiereRegion>[].obs;


  IdempiereRegionsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_REGIONS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_REGION_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_REGION;
    fields = IdempiereRestCommand.fieldsOfRegions.obs;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.REGIONS;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereRegion productBrand = IdempiereRegion.fromJson(item);
        results.add(productBrand);
      }
      print('----------------regions.length: ${results.length}');
      saveIdempiereRegionsList(results);
