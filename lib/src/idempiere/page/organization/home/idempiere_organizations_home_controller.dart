...
class IdempiereOrganizationsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereOrganization> results = <IdempiereOrganization>[].obs;


  IdempiereOrganizationsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_ORGANIZATIONS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_ORGANIZATION_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_ORGANIZATION;
    fields = IdempiereRestCommand.fieldsOfOrganizations.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereOrganization productBrand = IdempiereOrganization.fromJson(item);
        results.add(productBrand);
      }
      print('----------------organizations.length: ${results.length}');
      saveIdempiereOrganizationsList(results);
