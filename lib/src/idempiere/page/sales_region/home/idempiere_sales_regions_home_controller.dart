...
class IdempiereSalesRegionsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereSalesRegion> results = <IdempiereSalesRegion>[].obs;


  IdempiereSalesRegionsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_SALES_REGIONS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_SALES_REGION_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_SALES_REGION;
    fields = IdempiereRestCommand.fieldsOfSalesRegions.obs;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.SALES_REGIONS;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereSalesRegion productBrand = IdempiereSalesRegion.fromJson(item);
        results.add(productBrand);
      }
      print('----------------SalesRegions.length: ${results.length}');
      saveIdempiereSalesRegionsList(results);
