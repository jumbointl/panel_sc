...
class IdempiereProductsCategoriesHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereProductCategory> results = <IdempiereProductCategory>[].obs;


  IdempiereProductsCategoriesHomeController(){
    endPoint = Memory.IDEMPIERE_ENDPOINT_PRODUCTS_CATEGORY;
    routePageDestination = Memory.ROUTE_IDEMPIERE_PRODUCTS_CATEGORIES_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_PRODUCT_CATEGORY_LIST;
    fields = IdempiereRestCommand.fieldsOfProductsCategories.obs;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereProductCategory data = IdempiereProductCategory.fromJson(item);
        results.add(data);
      }
      print('----------------productsCategories.length: ${results.length}');
      saveIdempiereProductsCategoriesList(results);
