...
class IdempiereProductsLinesHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereProductLine> results = <IdempiereProductLine>[].obs;


  IdempiereProductsLinesHomeController(){
    endPoint = Memory.IDEMPIERE_ENDPOINT_PRODUCTS_LINE;
    routePageDestination = Memory.ROUTE_IDEMPIERE_PRODUCTS_LINES_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_PRODUCT_LINE_LIST;
    fields = IdempiereRestCommand.fieldsOfProductsLines.obs;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.PRODUCTS_LINES;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereProductLine data = IdempiereProductLine.fromJson(item);
        results.add(data);
      }
      print('------------------------------result.length: ${results.length}');
      saveIdempiereProductsLinesList(results);
