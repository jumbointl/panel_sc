...
class IdempiereTenantWithDetailsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereTenantWithDetail> results = <IdempiereTenantWithDetail>[].obs;


  IdempiereTenantWithDetailsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_TENANT_WITH_DETAILS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_TENANT_WITH_DETAIL_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_TENANT;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.TENANT_WITH_DETAILS;
    fields = IdempiereRestCommand.fieldsOfTenantWithDetails.obs;
  ...
  void goToTenantRolHomePage(String page){
  }
  @override
  void goToRolesPage(){
  }
  ...
  Future<void> findByCondition(BuildContext context) async {
    ...
    if(endPoint==null){
      ...
      for(var item in fromJson.records!){
        IdempiereTenantWithDetail list = IdempiereTenantWithDetail.fromJson(item);
        results.add(list);
      }
      print('-----------------------------.length: ${results.length}');
      saveIdempiereTenantWithDetailsList(results);
