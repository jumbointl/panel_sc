...
class IdempiereLocatorsHomeController extends IdempiereFilterCreateControllerModel{
  ...
  List<IdempiereLocator> results = <IdempiereLocator>[].obs;


  IdempiereLocatorsHomeController(){
    routePageDestination = Memory.ROUTE_IDEMPIERE_LOCATORS_LIST_PAGE;
    getStorageSaveKey = Memory.KEY_IDEMPIERE_LOCATOR_LIST;
    endPoint = Memory.IDEMPIERE_ENDPOINT_LOCATOR;
    fields = IdempiereRestCommand.fieldsOfLocators.obs;
    bottomNavigationBarColor = Colors.cyan[200]!;
    title = Messages.LOCATORS;
  ...
  Future<void> findByCondition(BuildContext context) async {
      ...
      for(var item in fromJson.records!){
        IdempiereLocator productBrand = IdempiereLocator.fromJson(item);
        results.add(productBrand);
      }
      print('----------------locators.length: ${results.length}');
      saveIdempiereLocatorsList(results);
