...
import 'places_handling_controller.dart';

class PlacesHandlingPage extends StatelessWidget {
  PlacesHandlingController controller = Get.put(PlacesHandlingController());
  bool createPage;
  PlacesHandlingPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(Messages.PLACES,style: TextStyle(color: Colors.white,
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              if(!createPage)_dropDownPlaces(controller.placesResults),
              _textFieldId(),
              _textFieldName(),
            ],
          ),
  ...
  Widget _dropDownPlaces(List<Place> places) {
    return Container(
      ...
      child: DropdownButton(
        ...
        isExpanded: true,
        hint: Text(
          places.isEmpty ?Messages.NO_DATA_FOUND : '${Messages.SELECT_A_VALUE} ${places.length})'
          ,
        ...
        items: _dropDownPlacesItems(places),
        value: controller.idPlacesResult.value == '' ? null : controller.idPlacesResult.value,
        onChanged: (option) {
          controller.idPlacesResult.value = option.toString();
          int? id =int.tryParse(option.toString());
          if(id!=null && id>0){
            Place? ob = controller.getPlaceById(controller.placesResults, id);
  ...
  List<DropdownMenuItem<String>> _dropDownPlacesItems(List<Place> Places) {
    List<DropdownMenuItem<String>> list = [];
    for (var places in Places) {
      list.add(DropdownMenuItem(

        value: places.id.toString(),
  ...
  Widget _textFieldName(){
    return Container(
      ...
      child: TextField(
        ...
        maxLength: MemoryPanelSc.MAX_LENGTH_PLACE_NAME,
